  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-5">
      <div className="w-full max-w-xl rounded-[32px] border border-[var(--line-soft)] bg-[var(--surface)] p-8 text-center shadow-[var(--shadow-panel)]">
        ...
        <h1 className="mt-6 text-3xl font-bold text-[var(--text-strong)]">
          {isTurkish ? 'Rota bulunamadi' : 'Route not found'}
        </h1>
        <p className="mt-3 text-sm leading-6 text-[var(--text-muted)]">
          {isTurkish
            ? 'Istedigin sayfa mevcut BADHABINOT on yuz rota haritasinda yok.'
            : 'The page you requested does not exist in the current BADHABINOT frontend route map.'}
        </p>
        <div className="mt-8 flex justify-center">
          <Link to="/">
            <Button>{isTurkish ? 'Calisma alanina don' : 'Return to workspace'}</Button>
          </Link>
        </div>
      </div>
    </div>
